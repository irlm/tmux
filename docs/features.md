...
The left side shows the session name, current git branch when present, and sync state. While a remote tmux is in front it switches to a `REMOTE` indicator, meaning the local prefix is off and keys are going to the server.
